...
def simple_prediction(x, y, clf, Method_Name, class_name, data_set, label_indexes, data_source='unknown', cross_val=10):
    mean_accuracy = 0
    mean_confusion_matrix = np.zeros((len(class_name), len(class_name)))
    clf_rf = clf
    cv = StratifiedKFold(10)
    label_indexes1 = label_indexes
    label_indexes1 = np.array(label_indexes1.index[label_indexes1['dataset'] == data_set].tolist())
    for train_index, test_index in cv.split(x, y):
        prediction_model = clf_rf.fit(x[train_index], y[train_index])
        test_index = np.intersect1d(test_index, label_indexes1)
        prediction = prediction_model.predict(x[test_index])
        prediction_probs = prediction_model.predict_proba(x[test_index]) # Alton added
        accuracy = accuracy_score(y[test_index], prediction)
        mean_accuracy += accuracy
        mean_confusion_matrix += confusion_matrix(y[test_index], prediction)
    ...
    mean_confusion_matrix = mean_confusion_matrix.astype(np.int16)
    plt.suptitle(data_source + ' with Liar - Accuracy:' + str(round(mean_accuracy,2)))
    plot_confusion_matrix(mean_confusion_matrix, classes=['True', 'Fake'])
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
...
### Running Individual Models for each dataset

data = pd.read_csv(csv_file)
label1 = data[['dataset']]
label1['dataset'].value_counts()
data = data.drop(['dataset'], axis=1)
...
simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Buzzfeed', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='NoLiar_Buzzfeed')

simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Risdal-McIntire', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='NoLiar_Risdal-McIntire')

simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Rosas-Celebrity', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='NoLiar_Rosas-Celebrity')

simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Rosas-News', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='NoLiar_Rosas-News')

### Including the Wang Dataset ------------------------------------------------

### Running Individual Models for each dataset
# csv_files = ['data_Wang_expandedfeatures_balanced_20compress.csv']

data = pd.read_csv('data_Wang_expandedfeatures_balanced_20compress.csv')
label1 = data[['dataset']]
label1['dataset'].value_counts()
data = data.drop(['dataset'], axis=1)
...
Method_Name = 'Adaboost_RandomForest'
simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Buzzfeed', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='Liar_Buzzfeed')

simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Risdal-McIntire', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='Liar_Risdal-McIntire')

simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Rosas-Celebrity', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='Liar_Rosas-Celebrity')

simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Rosas-News', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='Liar_Rosas-News')


# The liar dataset is basically random
simple_prediction(x=x, y=y_encode, clf=clf, data_set = 'Wang-PolitiFact', label_indexes=label1, Method_Name=Method_Name, class_name=class_name,
                  data_source='Liar_Liar')
